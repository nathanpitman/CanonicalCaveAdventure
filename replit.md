...
- **Natural Language Processing (NLP):** A dedicated NLP layer (`commandParser.ts`, `actionResolver.ts`) handles user input, transforming raw strings into game actions, supporting complex phrases and typo tolerance. Two complementary fuzzy matching algorithms work together: (1) **Damerau-Levenshtein distance** (`editDistanceAtMost2`) for standard edit-distance typo correction (up to 2 edits), and (2) **Keyboard-aware weighted edit distance** (`keyboardAwareEditDistance`) which uses a QWERTY adjacency map to penalize substitutions between neighboring keys at 0.5 cost instead of 1.0 — so typos like "pit" → "out" (p↔o and i↔u are adjacent keys) are recognized as close matches. The keyboard-aware score is the primary ranking signal with a tunable threshold (`MAX_FUZZY_DISTANCE = 1.5`), falling back to standard Levenshtein when keyboard proximity doesn't apply. Fuzzy matching is constrained to valid candidates for the current game state (available exits, inventory items). Includes `NARRATIVE_SYNONYMS` in `lexicon.ts` mapping ~50 natural language phrases (downstream, upstream, follow stream, etc.) to canonical action tokens (downs, upstr, bed, etc.). Comprehensive verb synonyms cover 15+ natural alternatives per action verb (e.g., catch/capture/nab/snatch/seize all map to "take"; shatter/crack/crush/bust all map to "break").
- **Contextual Exit Hints:** After each room description, a subtle nav-hint message (styled italic, dimmed) automatically lists available directions in natural language (e.g., "Paths lead north, east, and down."). Suppressed in dark rooms. Uses the `nav-hint` message type in `MessageBubble`.
...
- **Data-Driven Architecture:** The game is designed to be fully data-driven from `import/adventure.yaml`. The importer generates three files:
  ...
  - `generatedConstants.ts`: Object state constants, dwarf config (start locs, count, pirate index), direction/noun synonyms from YAML motions/objects vocabulary, verb synonyms from YAML actions vocabulary + NLP extensions (`YAML_VERB_SYNONYMS`, `YAML_MOVE_VERBS`, `YAML_TARGET_FIRST_VERBS`), maze scene IDs from location conditions, score classes, story messages.
...
- **Verb Synonym Generation:** The YAML importer generates verb synonyms from two sources: (1) YAML `actions` vocabulary (e.g., CARRY words: carry/take/catch/steal/etc.) and (2) NLP extensions defined in `VERB_NLP_EXTENSIONS` within the importer (e.g., grab/pocket/pluck/snatch added to CARRY). Future game adaptations should update `VERB_NLP_EXTENSIONS`, `EXTRA_VERB_GROUPS`, `ACTION_TO_VERB`, and the target-first/move verb configurations in the importer if new verbs or actions are introduced. Run `npx tsx tools/testVerbSynonyms.ts` after re-import to verify synonym completeness (319 baseline assertions).
...
- **Command Set:** Supports commands: `look`, `inventory`, `take`, `use`, `go`, `go back`, `drop`, `score`, `brief`, `wait`, `attack`, `throw`, `feed`, `wave`, `fill`, `pour`, `drink`, `read`, `open`, `unlock`, `close`, `lock`, `say`, `eat`, `rub`, `quit`, `blast`, `listen`, `fee/fie/foe/foo`.
- **Listen Command:** Displays ambient sound descriptions for the current location based on the scene's `sound` property (e.g., `STREAM_GURGLES` → "The stream is gurgling placidly."). Falls back to "All is silent." for scenes without a sound. Sound text mappings are in `SOUND_MESSAGES` in `generatedStory.ts`. Scenes with `loud: true` (volcano view, reservoir bottom) indicate sounds that drown out all other noise.
